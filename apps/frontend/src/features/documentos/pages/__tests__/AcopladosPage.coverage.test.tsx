/**
 * Tests de cobertura para AcopladosPage
 */
...
describe('AcopladosPage - Coverage', () => {
  ...
  beforeAll(async () => {
    ...
    await jest.unstable_mockModule('../../api/documentosApiSlice', () => ({
      useGetAcopladosQuery: () => ({
        data: { acoplados: [], pagination: { total: 0, page: 1, limit: 10 } },
        isLoading: false,
        error: null,
      }),
      useGetDadoresQuery: () => ({
      ...
      useUpdateAcopladoMutation: () => [jest.fn(), { isLoading: false }],
      useDeleteAcopladoMutation: () => [jest.fn(), { isLoading: false }],
    }));
    ...
    Component = module.default || module.AcopladosPage;
  });
