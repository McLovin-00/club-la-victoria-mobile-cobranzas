...
// Tipos para eventos de auditoría
...
/**
 * Hook principal para auditoría de usuarios
 */
export const useUserAudit = () => {
  ...
  const processBatch = useCallback(() => {
    ...
    if (import.meta.env.MODE === 'production') {
      ...
      fetch('/api/audit/batch', {
        ...
        Logger.error('Error enviando eventos de auditoría:', error);
      });
    }
  }, []);
  ...
  const auditSearch = useCallback(
    (searchTerm: string, resultsCount: number, filters?: Record<string, any>) => {
