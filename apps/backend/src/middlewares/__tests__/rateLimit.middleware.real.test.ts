...
describe('rateLimit.middleware (real)', () => {
  it('loginRateLimiter keyGenerator/skip/handler', () => {
    ...
    expect((loginRateLimiter as any).keyGenerator(req1)).toBe('9.9.9.9');

    const req2: any = { headers: { 'x-real-ip': '8.8.8.8' }, ip: '2.2.2.2', body: {} };
    expect((loginRateLimiter as any).keyGenerator(req2)).toBe('8.8.8.8');

    const req3: any = { headers: {}, ip: '7.7.7.7', path: '/health', body: {} };
    expect((loginRateLimiter as any).skip(req3)).toBe(true);
    req3.path = '/login';
    expect((loginRateLimiter as any).skip(req3)).toBe(false);

    const res: any = { status: jest.fn().mockReturnThis(), json: jest.fn().mockReturnThis() };
    (loginRateLimiter as any).handler(req1, res);
  ...
  it('passwordChangeRateLimiter keyGenerator uses forwarded-for or ip', () => {
    ...
    expect((passwordChangeRateLimiter as any).keyGenerator(req1)).toBe('1.2.3.4');
    const req2: any = { headers: {}, ip: '9.9.9.9' };
    expect((passwordChangeRateLimiter as any).keyGenerator(req2)).toBe('9.9.9.9');
  });

  it('apiRateLimiter keyGenerator/skip', () => {
    const req1: any = { headers: { 'x-forwarded-for': '5.5.5.5' }, ip: '9.9.9.9', path: '/api/health' };
    expect((apiRateLimiter as any).keyGenerator(req1)).toBe('5.5.5.5');
    expect((apiRateLimiter as any).skip(req1)).toBe(true);
    req1.path = '/socket.io';
    expect((apiRateLimiter as any).skip(req1)).toBe(true);
    req1.path = '/api/x';
    expect((apiRateLimiter as any).skip(req1)).toBe(false);
  });
});
