/**
 * Proposito: validar restricciones del rol admin interno sobre rutas operativas de remitos.
 */
...
async function expectRoleNavigation(pageUrl: string, page: Page): Promise<void> {
  ...
  await expect(page).toHaveURL(/\/portal\/admin-interno|\/admin/i);
}

test.describe('REM-E2E-006 - Permisos remitos (Admin Interno)', () => {
  test('admin interno puede acceder a listado de remitos', async ({ page }) => {
    await expectRoleNavigation('/remitos', page);
  });

  test('admin interno no opera ruta legacy /remitos/carga', async ({ page }) => {
    await page.goto('/remitos/carga', { waitUntil: 'domcontentloaded' }).catch(() => {});
    await expect(page).not.toHaveURL(/\/remitos\/carga/i);
    await expect(page).toHaveURL(/\/portal\/admin-interno|\/admin|\/remitos/i);
  });
});
