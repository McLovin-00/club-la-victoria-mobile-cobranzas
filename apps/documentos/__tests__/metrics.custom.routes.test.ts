...
jest.mock('../src/services/performance.service', () => ({
  performanceService: {
    getOptimizedCriticalAlerts: jest.fn().mockResolvedValue([{ entityType: 'CHOFER' }, { entityType: 'CAMION' }]),
    getSystemMetrics: jest.fn().mockResolvedValue({ databaseConnections: 0, materializedViewAge: 0 }),
