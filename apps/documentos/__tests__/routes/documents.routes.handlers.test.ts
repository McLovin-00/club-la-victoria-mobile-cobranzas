...
const findHandler = (method: 'get' | 'post' | 'delete', path: string) => {
...
describe('documents.routes handlers', () => {
  ...
  it('POST /normalize-expirations actualiza expiraciones nulas', async () => {
    prismaMock.document.updateMany.mockResolvedValueOnce({ count: 3 });
    const handler = findHandler('post', '/normalize-expirations');
    const res = createRes();
    const req = {} as Request;
    await handler(req, res);
    expect(prismaMock.document.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { expiresAt: null } })
    );
    expect(res.json).toHaveBeenCalledWith({ success: true, data: { updated: 3 } });
  });
});
