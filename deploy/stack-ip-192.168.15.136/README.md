# Despliegue BCA (sin Nginx) - IP fija 10.8.10.20
...
./keys/ (colocar jwt_public.pem y jwt_private.pem)
```

## Variables de entorno (aplicadas en el compose)
- Frontend/CORS: `FRONTEND_URLS=http://10.8.10.20:8550`, `CORS_ORIGIN=http://10.8.10.20:8550`
- Backends (en frontend build args): `VITE_API_URL=http://10.8.10.20:4800`, `VITE_DOCUMENTOS_API_URL=http://10.8.10.20:4802`, `VITE_DOCUMENTOS_WS_URL=http://10.8.10.20:4802`
- MinIO: `MINIO_PUBLIC_BASE_URL=http://10.8.10.20:9000`, `MINIO_INTERNAL_BASE_URL=http://minio:9000`, `MINIO_ENDPOINT=minio:9000`
- Flowise: `FLOWISE_ENDPOINT=http://10.8.10.20:3005/api/v1/extract`

## Despliegue
```bash
cd deploy/stack-ip-192.168.15.136
# Claves JWT (requerido por Documentos)
mkdir -p keys && cp /ruta/privada/jwt_public.pem keys/ && cp /ruta/privada/jwt_private.pem keys/
# Construir imágenes locales (o puedes usar CI/registry si ya existen)
...
- Backend: `curl http://10.8.10.20:4800/health`
- Documentos: `curl http://10.8.10.20:4802/health`
- MinIO: `curl http://10.8.10.20:9000/minio/health/ready`
- Flowise: `curl http://10.8.10.20:3005/`

### Accesos
- Frontend: `http://10.8.10.20:8550`
- API Core: `http://10.8.10.20:4800`
- Documentos: `http://10.8.10.20:4802`
- MinIO (web/API): `http://10.8.10.20:9000`
- Flowise: `http://10.8.10.20:3005`
- Postgres externo: `postgresql://evo:phoenix@10.8.10.20:5432/monorepo-bca`
...
- `DOCS_MAX_DEPRECATED_VERSIONS=3` alinea retención de copias.
- Ajustá `PDF_RASTERIZE_MAX_CONCURRENCY` según carga; valor inicial 3.
